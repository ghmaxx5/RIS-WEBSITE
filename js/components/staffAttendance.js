...
export function renderStaffAttendance() {
  ...
  return `
    <div class="space-y-6">
      ...
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 class="text-2xl font-bold text-slate-900 dark:text-white font-heading">
            Staff Attendance & Leave Roster
          </h1>
          <p class="text-slate-500 dark:text-slate-400 text-sm">
            ${isAdmin ? 'Track real-time teacher presence and process leave applications.' : (isTeacher ? 'Check in for duty and submit absence leave requests.' : 'Faculty directory and daily staff presence (Read Only).')}
          </p>
        </div>
      ...
      <!-- Main Layout -->
      <div class="grid grid-cols-1 ${isStudent ? '' : 'lg:grid-cols-3'} gap-6">
        
        <!-- Daily Staff Roster Card -->
        <div class="glass-card p-6 ${isStudent ? 'w-full' : 'lg:col-span-2'} space-y-4">
    ...
    <!-- LEAVE REQUEST MODAL (TEACHERS ONLY) -->
    ${isTeacher ? `
