...
export function renderNavbar() {
  ...
  return `
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex items-center justify-between h-16">
          ...
          <div class="flex items-center gap-2 sm:gap-3">
            ...
            <button onclick="window.openDbModal()" class="btn ${db.isConnected ? 'bg-emerald-500/20 text-emerald-300 border-emerald-500/40' : 'bg-slate-800 text-slate-300 border-slate-700'} text-[11px] px-2.5 py-1 rounded-full border flex items-center gap-1 font-semibold">
              <span class="w-2 h-2 rounded-full ${db.isConnected ? 'bg-emerald-400 animate-pulse' : 'bg-amber-400'}"></span>
              <span class="hidden md:inline">${db.isConnected ? 'Cloud DB Active' : 'Cloud DB Config'}</span>
            </button>

            <!-- Join App / Register Button -->
            <button onclick="window.openRegistrationModal()" class="btn bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-600 hover:to-teal-700 text-white text-xs px-3.5 py-1.5 rounded-full shadow-md font-bold flex items-center gap-1.5">
    ...
    <div id="db-config-modal" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-slate-950/75 backdrop-blur-sm p-4">
    ...
    <div id="registration-modal" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-slate-950/75 backdrop-blur-sm p-4">
